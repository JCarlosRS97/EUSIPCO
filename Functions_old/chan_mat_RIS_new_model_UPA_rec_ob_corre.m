function [Hdir,H1,H2] = chan_mat_RIS_new_model_UPA_rec_ob_corre(Nt_y,Nt_z,Nr_y,Nr_z,Nris_x,Nris_y,lt,lr,D,no_mat,Kdir,K1,K2,f,dist_ris,varargin)
...
% Corr_mat_tx = zeros(Nt,Nt);
% for i = 1:Nt
%     for ii = 1:Nt
%         temp1 = tx_arr(:,i);
%         temp2 = tx_arr(:,ii);
%         Corr_mat_tx(i,ii) = sinc( 2* norm(temp1-temp2)/ lambda );
%     end
% end
Corr_mat_tx = eye(Nt);
% RX array
...
% Corr_mat_rx = zeros(Nr,Nr);
% for i = 1:Nr
%     for ii = 1:Nr
%         temp1 = rx_arr(:,i);
%         temp2 = rx_arr(:,ii);
%         Corr_mat_rx(i,ii) = sinc( 2* norm(temp1-temp2)/ lambda );
%     end
% end
Corr_mat_rx = eye(Nr);
...
Corr_mat_ris = zeros(Nris,Nris);
for i = 1:Nris
    for ii = 1:Nris
        temp1 = RISPosition(:,i);
        temp2 = RISPosition(:,ii);
        Corr_mat_ris(i,ii) =  sinc( 2* norm(temp1-temp2)/ lambda );
    end
end
...
temp =1 ;
Hdir = Rician_ewise(Hdir_los,no_mat,Kdir, sqrtm(Corr_mat_rx), sqrtm(Corr_mat_tx));
H1 = Rician_ewise(H1_los*temp,no_mat,K1,sqrtm(Corr_mat_ris), sqrtm(Corr_mat_tx));
H2 = Rician_ewise(H2_los/temp,no_mat,K2,sqrtm(Corr_mat_rx), sqrtm(Corr_mat_ris));

function Hout = Rician_ewise(Hlos,no_mat,K,CorreMat1,CorreMat2)
    for ind = 1:no_mat
%         [Mm,Nn] = size(Hlos);
        Hnlos = CorreMat1*sqrt(1/2)*(randn(size(Hlos))+1i*randn(size(Hlos)))*CorreMat2;
