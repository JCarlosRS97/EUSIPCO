function plot_ris_cap_noDIR_mod(Nt,Nr,Nris,SNR,K,f,D,dist_ris,lr,lt,Pt) 
% clear; close all;
% Nt = 8;
% Nr = 4;
% Nris = 15^2;
...
%     [dC6,~,~] = GPM_acc_rescale_modified2(Nt,Nr,Nris,lt ,lr ,1,D,quant_bits,Pt,...
%          Hdir1*sqrt(SNR)/c,H1*sqrt(SNR),H2,no_iter,Qinit*c^2,omega_init/c,c);
    C6 = C6+dC6;  
    % only  DIR
    c=1;
    H11= zeros(size(H1));
