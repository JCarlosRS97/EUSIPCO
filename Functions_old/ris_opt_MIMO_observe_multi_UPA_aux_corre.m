function [ARoutput,AR_K1000_output,AR_anom_output, AR_focusing_output, phase_output, ...
    ...
    = ris_opt_MIMO_observe_multi_UPA_aux_corre(Nt_y,Nt_z,Nr_y,Nr_z,Nris_x,Nris_y,SNR,Kdir,K1,K2,f,D,dist_ris,lr,lt,Pt, no_iter,no_mat)
% clear; close all;
% Nt = 8;
% Nr = 4;
% Nris = 15^2;
...
N_miller = ceil((lambda/2).^4*(Nt_y-1)*(Nt_z-1)*(Nr_z-1)*(Nr_y-1)/(lambda^2*D^2));
...
d1_center = sqrt((-dist_ris-RISPosition(1,:)).^2 + RISPosition(2,:).^2 +lt^2);
d2_center = sqrt((D-dist_ris-RISPosition(1,:)).^2 + RISPosition(2,:).^2 +lr^2);
Phase_RIS_focusing = - k*(d1_center+d2_center);
...
for i = 1:no_mat
    ...
    [U,S,V] = svds(H_comp, 40+4*N_miller);
