function [ARoutput,AR_K1000_output,AR_anom_output, AR_focusing_output, phase_output, ...
    ...
    = ris_opt_MIMO_observe_multi_UPA_direct_carlos(Nt_y,Nt_z,Nr_y,Nr_z,Nris_x,Nris_y,SNR,Kdir,K1,K2,f,D,dist_ris,lr,lt,Pt, no_iter,no_mat)
% clear; close all;
% Nt = 8;
% Nr = 4;
% Nris = 15^2;
...
H1_K1000 = zeros(size(H1t_K1000{1})); % Cancel the reflect
H2_K1000 = zeros(size(H2t_K1000{1})); % Cancel the reflect
...
RIS_phase_opt_K1000 = omega_init.';
...
for i = 1:no_mat
    ...
    H1 = zeros(size(H1)); % Cancel the reflect
    H2 = zeros(size(H2)); % Cancel the reflect
